...
class UserManagementControllerTest {
    ...
    @Test
    void listEmployees_rejectsUnauthenticatedRequests() throws Exception {
        mockMvc.perform(get("/api/admin/users"))
                .andExpect(result -> assertThat(result.getResponse().getStatus()).isIn(401, 403));

        verify(userManagementService, never()).listEmployees(org.mockito.ArgumentMatchers.any());
    }

    @Test
    @WithMockUser(roles = "DISPATCHER")
    void listEmployees_rejectsDispatcherRole() throws Exception {
        mockMvc.perform(get("/api/admin/users"))
                .andExpect(status().isForbidden());
    ...
    @Test
    void listEmployees_rejectsActorWithoutBakery() throws Exception {
        User actor = User.builder()
                .id(UUID.randomUUID())
                .email("admin@test.com")
                .passwordHash("hashedPw")
        ...
        mockMvc.perform(get("/api/admin/users")
                        .with(authentication(authToken(actor))))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.error").value("This endpoint requires a bakery-scoped account"));

        verify(userManagementService, never()).listEmployees(org.mockito.ArgumentMatchers.any());
    }

    @Test
    void listEmployees_allowsBakeryAdminWithBakery() throws Exception {
        UUID bakeryId = UUID.randomUUID();
        Bakery bakery = Bakery.builder()
                .id(bakeryId)
                .name("Tenant Bakery")
        ...
        User employee = User.builder()
                ...
                .bakery(bakery)
                .build();
        when(userManagementService.listEmployees(bakeryId)).thenReturn(List.of(actor, employee));

        mockMvc.perform(get("/api/admin/users")
                        .with(authentication(authToken(actor))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].email").value("admin@test.com"))
                .andExpect(jsonPath("$[1].email").value("driver@test.com"));

        verify(userManagementService).listEmployees(bakeryId);
    }
