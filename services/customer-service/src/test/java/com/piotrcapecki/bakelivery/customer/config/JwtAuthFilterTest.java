...
class JwtAuthFilterTest {
    ...
    @Test
    void filter_validToken_setsCustomerPrincipalAuthentication() throws Exception {
        UUID userId = UUID.randomUUID();
        UUID bakeryId = UUID.randomUUID();
        when(jwtUtil.parse("valid-token")).thenReturn(new JwtClaims("anna@x.pl", userId, bakeryId, "CUSTOMER"));
        ...
        assertThat(auth.getPrincipal()).isInstanceOf(CustomerPrincipal.class);
        CustomerPrincipal p = (CustomerPrincipal) auth.getPrincipal();
        assertThat(p.userId()).isEqualTo(userId);
        assertThat(p.bakeryId()).isEqualTo(bakeryId);
        assertThat(p.email()).isEqualTo("anna@x.pl");
        assertThat(auth.getAuthorities()).extracting(Object::toString).containsExactly("ROLE_CUSTOMER");
